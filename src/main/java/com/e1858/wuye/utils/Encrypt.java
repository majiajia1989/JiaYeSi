...
{
	private static String					base64Tab	= "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
	private static final Logger logger = Logger.getLogger("web");
	public static String MD5(String s)
	{
		try
		{
			return Util.byte2HexString(MessageDigest.getInstance("MD5").digest(s.getBytes(CommonConstant.ENCODING)));
		}
		catch (Exception ex)
		{
	...
	public static void setDes3Key(String str)
	{
		try
		{
			byte[] abyte1 = str.getBytes(CommonConstant.ENCODING);
	...
	public static byte[] baseEncoding(byte[] sc)
	{
		...
		if (srcLength > 3 || srcLength < 1) return null;
	...
	public static byte[] baseDecoding(byte[] sc)
	{
		...
		if (srcLength > 4 || srcLength < 2) return null;
	...
	public static byte bitMove(byte sbyte, int count)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			if (abit.get(i)) rbyte = (byte) (rbyte | bits[i]);
		}
		return rbyte;
	}
